"""V22 v4.1 — WORST CASE STRESS TEST. Runs against strongest gold uptrend (+34%)."""
...
# WORST CASE: 3-month strongest gold uptrend (+34%)
...
def load_worst_data():
    f15 = os.path.join(DATA_DIR, "XAUUSD_worst_M15.csv")
    f5 = os.path.join(DATA_DIR, "XAUUSD_worst_M5.csv")
    if not os.path.exists(f15):
        print(f"[!] Worst-case data not found at {f15}")
        return None, None
    ...
    # Ensure DatetimeIndex, not string index
    d15.index = pd.to_datetime(d15.index, utc=True)
    if not d5.empty:
        d5.columns = [c.lower() for c in d5.columns]
        d5.index = pd.to_datetime(d5.index, utc=True)
...
def run_backtest():
    print("=" * 70)
    print("  🚨 WORST CASE STRESS TEST: V22 v4.1 (SYMMETRIC EMA200)")
    print(f"  PERIOD: {START_DATE} to {END_DATE}")
    print("  MARKET: STRONGEST GOLD UPTREND IN 5 YEARS (+34%)")
    print("=" * 70)
    
    d15, d5 = load_worst_data()
    if d15 is None:
    ...
    print(f"\nM15={len(d15)} M5={len(d5) if not d5.empty else 0}")
    ...
    if not closed:
        print("  No trades executed (filters too strict).")
        print(f"\nFinal Balance: ${balance:.2f}")
        print(f"P&L: ${balance - STARTING_BALANCE:+.2f}")
        return
    ...
    print(); print("="*70)
    print(f"  🚨 WORST CASE STRESS TEST RESULTS")
    print(f"  Period: {START_DATE} to {END_DATE} (+34% Gold Uptrend)")
    print("="*70)
